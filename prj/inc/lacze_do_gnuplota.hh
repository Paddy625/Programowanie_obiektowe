...
class LaczeDoGNUPlota {
 protected:
  ...
  /*!
   * \brief Tworzy listę parametrów umożliwiających rysowanie dodatkowych
   * elementów
   *
   ...
   * \pre Parametr \e Polecenie powinien zawierać polecenie \e plot lub \e
   * splot, do którego będzie możliwe dopisanie dalszego ciągu. \param Polecenie
   * - polecenie rysowania, do którego mają być dopisane nazwy plików i
   * odpowiednie parametry dla polecenia plot. \param Sep - zawiera znak
   * separatora między poszczególnymi parametrami. Jeżeli parametry listy
   * przeszkód są generowane jako pierwsze, to zmienna ta musi być wskaźnikiem
   * do wskaźnika na łańcuch: " ".
   */
  ...
  /*!
   * \brief  Udostępnia informację czy mają być wyświetlane informacje o
   * błędach.
   *
 ...
 public:
  ...
  bool DodajNazwePliku(const char *NazwaPliku,
                       RodzajRysowania RodzRys = RR_Ciagly, int Szerokosc = 1);
...
inline bool LaczeDoGNUPlota::DopiszPlikiDoPoleceniaRysowania(std::string &,
                                                             char const **) {
  return true;
}
