...
bool LaczeDoGNUPlota::PrzeslijDoGNUPlota(const char *Komunikat) {
  ...
  while (
      ((IloscPrzeslanych = write(_Wejscie_GNUPlota, Komunikat, Ilosc)) != -1) &&
      Ilosc > 0) {
...
bool LaczeDoGNUPlota::DodajNazwePliku(const char *NazwaPliku,
                                      RodzajRysowania RodzRys, int Szerokosc) {
  bool rezultat = true;
...
bool LaczeDoGNUPlota::UtworzProcesPotomny() {
  ...
  switch (fork()) {
    ...
    case 0:
      ...
      KomunikatBledu(
          "!!! Do uruchomienia programu gnuplot bylo uzyte polecenie:");
      KomunikatBledu(Comm);
      exit(1);
...
bool LaczeDoGNUPlota::RysujDoPliku(const char *NazwaPliku) {
  ...
  if (!PrzeslijDoGNUPlota("\n\n\nset output \nset term x11\n")) {
    KomunikatBledu(
        "!!! Wyjscie dla gnuplot'a nie zostalo poprawnie\n"
...
bool LaczeDoGNUPlota::DopiszRysowanieZPlikow(std::string &Polecenie,
  ...
  for (; Nazwy != _InfoPlikow.end(); ++Nazwy) {
    ...
    if (Nazwy->WezRodzRys() == RR_Ciagly)
      OSStrm << " lw " << Nazwy->WezSzerokosc();
    else
      OSStrm << " ps " << Nazwy->WezSzerokosc();
...
void LaczeDoGNUPlota::BudujPreambulePoleceniaRysowania(
    std::string &Preambula) const {
  switch (_TrybRys) {
    case TR_2D:
...
std::string LaczeDoGNUPlota::ZapiszUstawienieRotacjiISkali() const {
  ostringstream strm;
  strm << "set view " << RotacjaX() << "," << RotacjaZ() << "," << SkalaX()
       << "," << SkalaZ() << endl;
  return strm.str();
}
...
//
void LaczeDoGNUPlota::UsunWszystkieNazwyPlikow() { _InfoPlikow.clear(); }
